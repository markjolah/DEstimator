 ...
 */
#ifndef DESTIMATOR_DESTIMATOR_H
#define DESTIMATOR_DESTIMATOR_H
...
public:
    using IdxT = uint32_t;
    using VecT = arma::Col<FloatT>; //!< vector type
    using MatT = arma::Mat<FloatT>; //!< matrix type
    
    IdxT Ndim; //!< Number of dimensions in trajectory
    IdxT N;    //!< Number of points in trajectory
     ...
     * @returns The log-likelihood: log(P(O|D))
     */
    FloatT LLHdim(FloatT D, IdxT dim); //Parallelized over Dim
     ...
     * @param[out] LLH - A vector of log-likelihood values for each of the given D values
     */
    void LLHdim(const VecT &D, IdxT dim, VecT &LLH); //Parallelized over D
...
private:
     ...
     */
    static FloatT coreLLH(IdxT N, const FloatT Obs[], const FloatT T[], const FloatT vD[], const FloatT vM[]);
