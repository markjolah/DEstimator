...
DEstimator<FloatT>::DEstimator(const MatT &Obs_, const VecT &T_, const MatT &SE_, FloatT exposureT_)
    : Ndim(static_cast<IdxT>(Obs_.n_cols)), N(static_cast<IdxT>(Obs_.n_rows)),
...
template<class FloatT>
FloatT DEstimator<FloatT>::LLHdim(FloatT D, IdxT dim)
{
...
{
    IdxT ND = static_cast<IdxT>(D.n_elem);
    LLH.zeros();
    #pragma omp parallel
    {
        VecT local_vD(N-1);
...
template<class FloatT>
void DEstimator<FloatT>::LLHdim(const VecT &D, IdxT dim, VecT &LLH)
{
    IdxT ND = static_cast<IdxT>(D.n_elem);
    if(dim<0 || dim>=Ndim) throw std::domain_error("dim>=Ndim || dim<0");
    FloatT *pObs=Obs.memptr()+N*dim; //Select out the "dim" column from the matrix of observations
...
{
    IdxT N = static_cast<IdxT>(Obs.n_elem);
    IdxT ND = static_cast<IdxT>(D.n_elem);
...
void DEstimator<FloatT>::LLH_laplace1D( const VecT &D, const VecT &Obs, const VecT &T, const VecT &SE, FloatT exposureT, VecT &LLH)
{
    ...
    #pragma omp parallel
    {
        ...
        for(IdxT k=0;k<ND;k++){
            ...
            IdxT vMsign = 1;
            for(IdxT n=0;n<N;n++) vMsign *= sgn(vM(n));
            //Compute hessian
            ivD = 1./vD;
            ivM = 1./vM;
            hess.a = -ivD;
            hess.b(0) = ivM(0)+ivD(0);
            for(IdxT n=1; n<N-1; n++) hess.b(n) = ivM(n)+ivD(n)+ivD(n-1);
            hess.b(N-1) = ivM(N-1)+ivD(N-2);
            hess.solve(Obs%ivM, theta);//Solve for most likely true particle positions, theta

            //Compute log likelihood
            for(IdxT n=0;n<N;n++) LLH(k) += square(Obs(n)-theta(n))*ivM(n);
            for(IdxT n=0;n<N-1;n++) LLH(k) += square(theta(n+1)-theta(n))*ivD(n);
            LLH(k) += (N-1)*log2pi;
            LLH(k) += logprod(vM);
            LLH(k) += logprod(vD);
...
void DEstimator<FloatT>::LLH_markov1D( const VecT &D, const VecT &Obs, const VecT &T, const VecT &SE, FloatT exposureT, VecT &LLH)
{
    ...
    #pragma omp parallel
    {
        ...
        for(IdxT k=0;k<ND;k++){
            ...
            for(IdxT n=0;n<N-1;n++) cov.b(n) = vD(n)+vM(n)+vM(n+1); //Central diagonal
...
template<class FloatT>
FloatT DEstimator<FloatT>::coreLLH(IdxT N, const FloatT Obs[], const FloatT dT[], const FloatT vD[], const FloatT vM[])
{
    VecT alpha(N-1);
    FloatT eta = vD[0]+vM[0];
    FloatT mu = Obs[0];
    FloatT LLH = 0;
    for(IdxT n=1;n<N-1;n++){
...
{
    IdxT N = static_cast<IdxT>(SE.n_elem);
    D=fabs(D);
    vD = 2*D*dT;
    for(IdxT n=0; n<N-1; n++) if(vD(n)<min_variance) vD(n)=min_variance;
